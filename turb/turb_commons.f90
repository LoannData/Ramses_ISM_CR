...
module turb_commons
  ...
  complex(cdp), allocatable :: turb_last(:,:,:,:)
                                       ! Turbulent spectrum at time = t
  complex(cdp), allocatable :: turb_next(:,:,:,:)
                                       ! Turbulent spectrum at time = t + dt
  real(dp), allocatable     :: afield_last(:,:,:,:)
                                       ! Forcing field at time = t
  real(dp), allocatable     :: afield_next(:,:,:,:)
                                       ! Forcing field at time = t + dt
  real(dp), allocatable     :: afield_now(:,:,:,:)
                                       ! Forcing field now
  real(dp), allocatable     :: power_spec(:,:,:)
