subroutine output_makefile(filename)
  ...
  write(ilun,format)"   NPSCAL := $(NPSCAL)+$(NIONS)+4 # for internal energy + current"
  write(ilun,format)"else"
  write(ilun,format)"   NPSCAL := $(NPSCAL)+$(NIONS)+1 # for internal energy"
  ...
  write(ilun,format)"## 9:9+nener : trapped photons (look at definitions in mhd/read_hydro_param)"
  write(ilun,format)"## 9+nener9+nener+1:9+nener+nextinct : for extenction (look at definition of firstindex_pscal in mhd/read_hydro_param)"
  ...
  write(ilun,format)"## tehn in rt_init nvar_count=ichem-1 and iIons = nvar_count+1 (this is terrible, I know....)"
  ...
  write(ilun,format)""
  write(ilun,format)"#then do not forgot that within the code the magnetic variable are dupplicated and stored"
  ...
  write(ilun,format)"	  -DNEXTINCT=$(NEXTINCT)  #-DRT  -DNIONS=$(NIONS) -DNGROUPS=$(NGROUPS)   #-DQUADHILBERT=$(QUADHILBERT)"
  ...
  write(ilun,format)"VPATH = $(shell [ -z $(PATCH) ] || find $(PATCH) -type d):../$(SOLVER):../fld:../nimhd:../aton:../rt:../hydro:../pm:../poisson:../amr"
  write(ilun,format)"#############################################################################"
  ...
  write(ilun,format)"	  radiation_parameters.o rt_parameters.o rt_hydro_commons.o coolrates_module.o \"
  write(ilun,format)"         rt_spectra.o  rt_metal_cooling_module.o rt_cooling_module.o rt_flux_module.o feedback_module.o"
  write(ilun,format)""
  ...
  write(ilun,format)"PATCH2OBJ = cooling_module_frig.o sink_RT_feedback.o ## \"
  write(ilun,format)""
  write(ilun,format)"ifeq ($(NIMHD),1)"
  ...
  write(ilun,format)""
  write(ilun,format)"# All objects"
  write(ilun,format)"AMRLIB = $(AMROBJ) $(HYDROOBJ) $(PMOBJ) $(POISSONOBJ) $(RTOBJ) $(PATCH2OBJ) # $(TURBOBJ)"
