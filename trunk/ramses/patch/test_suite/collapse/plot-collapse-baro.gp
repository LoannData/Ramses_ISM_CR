...
mu     = system(sprintf("grep mu_gas output_00001/info_00001.txt | cut -d '=' -f2"))
...
set logscale y2
set format y2 "10^{%L}"
plot file1 u (($7)*scale_d):(($17)*scale_d*((scale_l/scale_t)**2)*mu_gas*mh/(($7)*scale_d*kb)) every 20 w d axes x1y1, file1 u (($7)*scale_d):(sqrt(0.25*((($11)+($14))**2+(($12)+($15))**2+(($13)+($16))**2)*4.0*pi*scale_d*((scale_l/scale_t)**2))) every 20 w d lc 3 axes x1y2
