program amr2cube
  ...
  logical::ok,ok_part,ok_cell,blank_y=.false.,blank_z=.false.
  ...
  call read_params(blank_y,blank_z)
  ...
  read(10,*)
  read(10,*)
  read(10,'(A13,E23.15)')temp_label,scale_l
...
!                  rho = 0.5*(var(:,ind,5)**2+var(:,ind,6)**2+var(:,ind,7)**2)
  ...
  if(TRIM(filetype).eq.'txt')then
     ...
     open(unit=20,file=nomfich,form='formatted')
     do k=kmin,kmax
        zz=(dble(k-1)+0.5d0)/(2**lmax)*boxlen*scale_l
        do j=jmin,jmax
           yy=(dble(j-1)+0.5d0)/(2**lmax)*boxlen*scale_l
           do i=imin,imax
              xx=(dble(i-1)+0.5d0)/(2**lmax)*boxlen*scale_l
              write(20,'(10(es17.8e3,1x))') xx,yy,zz,real((grid(lmax)%cube(i,j,k)),8)
           enddo
           if(blank_y) write(20,*)
        enddo
        if(blank_z) write(20,*)
