...
subroutine accrete_sink(ind_grid,ind_part,ind_grid_part,ng,np,ilevel,on_creation)
  ...
  integer::i,j,nx_loc,isink,ivar,irad,idim,ind,ht,idust
  real(dp)::v2,d,e,d_floor,density,volume,eint,trmp,ekin,sum_dust
#ifdef SOLVERmhd
  real(dp)::bx1,bx2,by1,by2,bz1,bz2
#endif
     ...
     do j=1,np
        if(ok(j,ind))then
           ...
           if (on_creation)then
              ...
              if (threshold_accretion)then
                 ...
                 if(jeans_accretion)then
                    ...
                    call soundspeed_eos((1.0_dp-sum_dust)*d,eint,c2)
                    c2=c2**2
                    !tododust modifier d_jeans = d_jeans/ (1-sum_dust)
                    !d_jeans=c2*3.1415926*(1.0_dp-sum_dust)/(4.0*dx_loc)**2/factG
           ...
           if (on_creation)then
              ...
              call enerint_eos((1.0_dp-sum_dust)*d,temp,eint)
              e=eint
#ifdef SOLVERmhd
              e=e+0.125d0*((bx1+bx2)**2+(by1+by2)**2+(bz1+bz2)**2)
#endif
...
!              unew(indp(j,ind),5)=unew(indp(j,ind),5)-uold(indp(j,ind),5)*acc_mass/(d*vol_loc)+delta_e_tot/vol_loc
              ...
              d=unew(indp(j,ind),1)
              cv=eint/temp/d/(1.0_dp-sum_dust)
...
subroutine make_sink_from_clump(ilevel)
  ...
  integer ::ncache,nnew,ivar,irad,ngrid,icpu,index_sink,index_sink_tot,idust
  ...
  logical ::ok_free
  real(dp)::d,u,v,w,e,factG,delta_d,v2,eint,sum_dust
  real(dp)::birth_epoch
  real(dp)::dx,dx_loc,scale,vol_loc
     ...
     do igrid=1,ncache,nvector
        ...
        do ind=1,twotondim
           ...
           do i=1,nnew
              ...
              d=d-delta_d
!              e=e*d
              call enerint_eos((1.0_dp-sum_dust)*d,temp,e)
