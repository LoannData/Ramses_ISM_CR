...
subroutine coolfine1(ind_grid,ngrid,ilevel)
  ...
  !-------------------------------------------------------------------
  !-------------------------------------------------------------------
  integer::i,ind,iskip,idim,nleaf,nx_loc,ix,iy,iz,ivar,irad,neul=5
!  real(dp)::scale_nH,scale_T2,scale_l,scale_d,scale_t,scale_v
  ...
  real(kind=8),dimension(1:nvector),save::nH,T2,delta_T2,ekk,err,nh2
...
!!$#endif
           ...
           mincolumn_dens = 1.d50
           do index_m=1,NdirExt_m
              do index_n=1,NdirExt_n
                 vcolumn_dens(index_m,index_n)=column_dens_loc(ind_ll,index_m,index_n)          
                 mincolumn_dens=min(vcolumn_dens(index_m,index_n),mincolumn_dens)
              end do
           end do
...
!!$           coef = 1.0d0!2.d-21 *sc_l* boxlen       !cm^2; Eq.34 Glover & Mac Low 2007
           coef = 5.8d-22 *scale_l* boxlen       !cm^2; Semenov et al. 2010 or Eq.34 Glover & Mac Low 2007
...
!!$           uold(ind_leaf(i),8+nrad+nextinct) = coeff_chi
           uold(ind_leaf(i),8+nrad+nextinct) = -log(coeff_chi) *2.5d0
...
!!$#endif
                             H2column_dens(i,mloop,nl) = column_dens(i,mloop,nl)
                          end do
                       end do
...
!!$     coef = 2.d-21 *sc_l* boxlen       !cm^2; Eq.34 Glover & Mac Low 2007
     coef = 2.d-22 *sc_l* boxlen       !cm^2; Eq.34 Glover & Mac Low 2007
