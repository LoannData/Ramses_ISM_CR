subroutine courant_fine(ilevel)
  ...
  use hydro_commons
  use poisson_commons
  use radiation_parameters,only:frad,dtdiff_params,stellar_photon,dtmax
#if USE_TURB==1
  use turb_commons
#endif
  implicit none
...
!!$  if(stellar_photon .and. dtnew(ilevel) .ge. dtmax) then
!!$     if(myid == 1) write(*,*) 'WARNING: dt is limited by hand to dtmax=',dtmax
!!$     dtnew(ilevel)=dtmax
!!$  endif
