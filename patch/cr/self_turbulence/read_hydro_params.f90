subroutine read_hydro_params(nml_ok)
  ...
  ! Local variables
  !--------------------------------------------------
  integer::i,j,irad,nboundary_true=0,ht,idim,ivar
  integer ,dimension(1:MAXBOUND)::bound_type
  real(dp)::scale,ek_bound,em_bound,er_bound
  ...
  namelist/hydro_params/gamma,courant_factor,smallr,smallc,smallcr &
       ...
       & ,positivity_type,switch_solv,switch_solv_dens &
#if NENER>0
       & ,gamma_rad &
#endif
  ...
  namelist/physics_params/cooling,haardt_madau,metal,isothermal,barotrop,eos &
       ...
       & ,units_density,units_time,units_length,J21,a_spec,z_ave,z_reion,eta_mag,delayed_cooling,T2max &
       & ,self_shielding,smbh,agn,B_ave,t_diss,momentum_feedback &
!       & ,rsink_max,msink_max,merge_stars &
       & ,conduction,conduction_ion,twotemp,isotrope_cond,slopelim_cond,k_perp,cr_diffusion & !,f_spitzer
       ...
       & ,M0,Dcr,epsilon_diff_cr,epsilon_diff,fix_temp_diff,alfven_diff_coeff &
       & ,r_length,Dcr,saturation,coupling,coupling_out_of_conduction & ! testcase,
       & ,semi_implicit,frozen,flinj,Tfloor,RelVar,TCRmax,TCRmin &
       & ,streaming_diffusion,streaming_heating,Dmax,nlength_str,variable_diff_coeff, cooling_cr 
       !& ,cooling_cr  !---------------------------------------------------------------------------------------------------------!


  namelist/radiation_params/grey_rad_transfer,dtdiff_params,dt_control &
       & ,rosseland_params,planck_params,fld_limiter,epsilon_diff & !
