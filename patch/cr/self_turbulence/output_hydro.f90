subroutine file_descriptor_hydro(filename)
  ...
  if(variable_diff_coeff)then
   write(ilun,'("nvar        =",I11)')nvar+6   !------------------------------------------------- 5
else 
    write(ilun,'("nvar        =",I11)')nvar+4 !----------------------------------------------------- 3
end if  
...
#endif
  character(LEN=80)::filename
  integer::i,ivar,ncache,ind,ilevel,igrid,iskip,ilun,istart,ibound,ht,irad
     ...
     do ibound=1,nboundary+ncpu
        ...
        if(ncache>0)then
           ...
           do ind=1,twotondim
              ...
              if(variable_diff_coeff)then
               do ivar=1,2 ! Write Cr diffusion coeff
                  do i=1,ncache
                     xdp(i)=Dcrdiff(ind_grid(i)+iskip,ivar)
                     !if (xdp(i) < 0.) then 
                     !print *,"iskip = ",iskip
                     !print *,"Dcrdiff = ",Dcrdiff(ind_grid(i)+iskip,ivar)
                     !print *,"Dcrdiff = ",xdp(i)," ivar = ",ivar  ! -----------------------------------------------------------------
                     !endif
                  end do
                  write(ilun)xdp
               end do
