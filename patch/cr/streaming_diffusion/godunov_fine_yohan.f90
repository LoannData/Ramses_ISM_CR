...
subroutine set_uold(ilevel)
  ...
  do ind=1,twotondim
     ...
     do ivar=1,nvar+3
        do i=1,active(ilevel)%ngrid
!!$           if(ivar==9)write(*,'(A,i10,2e11.3)')'uold,unew',active(ilevel)%igrid(i)+iskip,uold(active(ilevel)%igrid(i)+iskip,ivar),unew(active(ilevel)%igrid(i)+iskip,ivar)
        ...
        do idim=1,ndim
           ...
           do i=1,ngrid
              if(igridn(i,ig1)>0)then
                 ...
                 if(streaming_heating)then
                    do igroup=1,ncr
                       Ercg(i,idim,igroup) = uold(igridn(i,ig1)+ih1,8+igroup)
                    end do
                 endif
#endif
              else
                 ...
                 if(streaming_heating)then
                    do igroup=1,ncr
                       Ercg(i,idim,igroup) = uold(ind_left(i,idim),8+igroup)
                    end do
                 endif
#endif
              end if
           ...
           do i=1,ngrid
              if(igridn(i,ig2)>0)then
                 ...
                 if(streaming_heating)then
                    do igroup=1,ncr
                       Ercd(i,idim,igroup) = uold(igridn(i,ig2)+ih2,8+igroup)
                    end do
                 endif
#endif
              else 
                 ...
                 if(streaming_heating)then
                    do igroup=1,ncr
                       Ercd(i,idim,igroup) = uold(ind_right(i,idim),8+igroup)
                    end do
                 endif
#endif
              end if
           ...
           do i=1,ngrid
              ...
              do igroup=1,ncr
                 if(bsign(igroup).eq.0.0d0)then
                    ...
                    do idim=1,ndim
                       vst(i,idim,igroup)=-bsign(igroup)*bc(idim)/sqrtrho
                    enddo
                 endif
              enddo
              do idim=1,ndim
...
#if NENER>NGRP .and. NCR>0
        if(streaming_heating)then
           do i=1,ngrid
              ! Add CR streaming heating term
              do igroup=1,ncr
                 unew(ind_cell(i),8+igroup)=unew(ind_cell(i),8+igroup) &
                      &  +(gamma_rad(igroup)-1.0d0)*vstgradEcr(i,igroup)*dtnew(ilevel)
                 unew(ind_cell(i),8+igroup)=MAX(unew(ind_cell(i),8+igroup),TCRmin)
              end do
           end do
        endif
#endif
