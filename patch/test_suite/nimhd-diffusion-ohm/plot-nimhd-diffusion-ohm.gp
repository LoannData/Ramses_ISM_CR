...
lmin = int(system(sprintf("grep levelmin nimhd-diffusion-ohm.nml | cut -d '=' -f2")))
lmax = int(system(sprintf("grep levelmax nimhd-diffusion-ohm.nml | cut -d '=' -f2")))
...
b     = 0.00108967
dx    = 0.5**lmax
f1(x) = 0.5*(erf((-(x-0.5-dx/2.0)+b/2)/sqrt(4*t))+erf(((x-0.5-dx/2.0)+b/2)/sqrt(4*t)))

set term post enh color portrait
set output 'nimhd-diffusion-ohm.ps'
...
set xrange[0.0:1.0]
set y2tics
set y2label 'error' rotate by -90 offset -2.0
