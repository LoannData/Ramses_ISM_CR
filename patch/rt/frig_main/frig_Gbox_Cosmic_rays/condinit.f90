...
#if NCR>0
!initialise cosmic rays pressure
     do ivar=1,ncr
        q(i,firstindex_ent+ivar)=q(i,5)*frac_Pcr
     end do
#endif
...
!     temper = q(i,5) / q(i,1)
       ...
       !now add a shear along the x-axis

       q(i,2) = q(i,2) + Vshear*(x(i,2)/boxlen)!*max(exp(-x(i,3)**2/(2.*Height0**2)),1.e-2) 

       ! CC 05/17 Nordita : Roberts flow
       !u_rf = 20. 
       !q(i,2) = u_rf*sin(2.*pi*x(i,2)/boxlen)*cos(2.*pi*x(i,1)/boxlen)
       !q(i,3) = u_rf*sin(2.*pi*x(i,1)/boxlen)*cos(2.*pi*x(i,2)/boxlen)
       !q(i,4) = u_rf*(1./sqrt(2.))*cos(2.*pi*x(i,1)/boxlen)*cos(2.*pi*x(i,2)/boxlen)
