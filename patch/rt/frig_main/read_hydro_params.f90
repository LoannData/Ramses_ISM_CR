...
!       & ,rsink_max,msink_max,merge_stars &
       ...
       & ,M0,Dcr,epsilon_diff_cr,fix_temp_diff,alfven_diff_coeff,NdirExt_m,NdirExt_n
...
108 continue
  ...
  !--------------------------------------------------
  ! Check for extinction and H2 variables
  !--------------------------------------------------
#if NEXTINCT > 1
     write(*,*) 'NPSCAL ',NPSCAL

#ifdef RT
     write(*,*) 'Take care, extinction and M1 have never been used together'
     STOP
#endif

#if NIMHD == 0 
#if NPSCAL < 2  
     write(*,*) 'Error: it is expected to have NPSCAL > 1 as the H2 is contained in NPSCAL'
#endif
#endif

#if NIMHD == 1
     write(*,*) 'Take care H2 physics never tested with non-ideal mhd, check !'
#if NPSCAL < 5  
     write(*,*) 'Error: it is expected to have NPSCAL > 4 as H2 is contained in NPSCAL'
#endif
#endif
