...
subroutine add_pdv_source_terms(ilevel)
  ...
  !---------------------------------------------------------
  integer::i,j,k,ivar,irad,ind,iskip,nx_loc,ind_cell1
...
subroutine add_shear_terms(ilevel)
  ...
  integer::ncache,igrid,ngrid,idim
  integer,dimension(1:3,1:2,1:8)::iii,jjj
     ...
     do ind=1,twotondim
        ...
        do idim=1,ndim
           ...
           do i=1,ngrid
              ...
              !explicit scheme
!              unew(ind_cell(i),2) = d*u + 2.*d*( v*Omeg_shear ) * dtnew(ilevel)
!              unew(ind_cell(i),3) = d*v + 2.*d*(-u*Omeg_shear + (xx(i,2)-0.5*boxlen)*Omeg_shear**2) * dtnew(ilevel)


              !implicit Crank Nicholson
              Ohmdt = Omeg_shear*dtnew(ilevel) 
              unew(ind_cell(i),2) = d*( u*(1.-Ohmdt**2) + 2.*v*Ohmdt + 2.*(xx(i,2)-0.5*boxlen)*Omeg_shear*Ohmdt**2 ) / (1.+Ohmdt**2)

              unew(ind_cell(i),3) = d*( v*(1.-Ohmdt**2) - 2.*u*Ohmdt + 2.*(xx(i,2)-0.5*boxlen)*Omeg_shear*Ohmdt ) / (1.+Ohmdt**2)

              unew(ind_cell(i),4) = d*w

              !write(*,*)'Force X'
              !write(*,*)2.*d*( v*Omeg_shear )
              !write(*,*)'Force Y'
!!              write(*,*)2.*d*(-u*Omeg_shear + (xx(i,2)-0.5*boxlen)*Omeg_shear**2)
              !write(*,*)'Position Y'
              !write(*,*)(xx(i,2)-0.5*boxlen)
              !write(*,*)'Position Y calculee a partir de la vitesse V_X'
              !write(*,*)u/Omeg_shear

              e_kin = 0.5*(unew(ind_cell(i),2)**2+unew(ind_cell(i),3)**2+unew(ind_cell(i),4)**2) / d 

!              enew(ind_cell(i))  = eold + e_kin
