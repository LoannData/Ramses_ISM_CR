...
subroutine set_uold(ilevel)
  ...
  !YN mars 2018 for CAI as passive scalars----------------------------------- 
#if NPSCAL>0
  integer  :: ht
#endif
  !--------------------------------------------------------------------------
...
#if NPSCAL>=3
     !--------------------------------------------------------------------
           if(energy_fix) call temperature_eos(d,e_prim,Tp_loc,ht)
           Tp_loc = max(Tp_loc, 0.3d0*Tr_floor) 
           if(Tp_loc .ge. 1650.d0)then
              uold(ind_cell,firstindex_pscal+2)=uold(ind_cell,firstindex_pscal+2)+&
                 uold(ind_cell,firstindex_pscal+1)+uold(ind_cell,firstindex_pscal+3)
              uold(ind_cell,firstindex_pscal+1) = 0.d0
              uold(ind_cell,firstindex_pscal+3) = 0.d0
           else if(Tp_loc .lt. 1650.d0 .and. Tp_loc .ge. 1500.d0)then
              uold(ind_cell,firstindex_pscal+3)=uold(ind_cell,firstindex_pscal+3)+&
              uold(ind_cell,firstindex_pscal+2)
              uold(ind_cell,firstindex_pscal+2) = 0.d0
           else
              uold(ind_cell,firstindex_pscal+1)=uold(ind_cell,firstindex_pscal+1)+&
              uold(ind_cell,firstindex_pscal+2)
              uold(ind_cell,firstindex_pscal+2) = 0.d0
           end if
#else
     write(*,*)'NPSCAL must >= 3 for CAI calculations'
     call clean_stop
#endif
#endif
     !--------------------------------------------------------------------
