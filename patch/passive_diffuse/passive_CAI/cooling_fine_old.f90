...
subroutine coolfine1(ind_grid,ngrid,ilevel)
  ...
  ! fev 2018 YN added to compute CAI formation
  !--------------------------------------------------------------------
  real(dp) :: d,e,T,ht
  !--------------------------------------------------------------------
...
#if NPSCAL>0
#if NPSCAL>=4
     ! fev 2018 YN added to compute CAI formation: dust, gas, CAI
     !--------------------------------------------------------------------
     do i=1,nleaf
       d = max(uold(ind_leaf(i),1),smallr)
!       e = uold(ind_leaf(i),ndim+2) - ekk(i) - err(i) - emag(i)
!       call temperature_eos(d,e,T2(i),ht)
       e = uold(ind_leaf(i),nvar)
       call temperature_eos(d,e,T,ht)
     !  T2(i) = uold(ind_leaf(i),ndim+2) - ekk(i) - err(i) - emag(i)
     !  T2(i) = T2(i)*nH(i)/scale_T2/(gamma-1.0) 
       if(T .ge. 1650.)then
         uold(ind_leaf(i),firstindex_pscal+2)=uold(ind_leaf(i),firstindex_pscal+2)+&
           uold(ind_leaf(i),firstindex_pscal+1)+uold(ind_leaf(i),firstindex_pscal+3)
         uold(ind_leaf(i),firstindex_pscal+1) = 0.
         uold(ind_leaf(i),firstindex_pscal+3) = 0.
       else if(T .lt. 1650. .and. T .ge. 1500.)then
         uold(ind_leaf(i),firstindex_pscal+3)=uold(ind_leaf(i),firstindex_pscal+3)+&
           uold(ind_leaf(i),firstindex_pscal+2)
         uold(ind_leaf(i),firstindex_pscal+2) = 0.
       else
         uold(ind_leaf(i),firstindex_pscal+1)=uold(ind_leaf(i),firstindex_pscal+1)+&
           uold(ind_leaf(i),firstindex_pscal+2)
         uold(ind_leaf(i),firstindex_pscal+2) = 0.
       end if
     end do
#else
     write(*,*)'NPSCAL must >= 4 for CAI calculations'
     call clean_stop
#endif
#endif
     !--------------------------------------------------------------------
