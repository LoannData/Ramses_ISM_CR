...
#if NPSCAL>=2
     !--------------------------------------------------------------------
           if(energy_fix) call temperature_eos(d,e_prim,Tp_temp,ht)
           Tp_temp = max(Tp_temp, 0.3d0*Tr_floor)
           if(Tp_temp .ge. 1500.d0) uold(ind_cell,firstindex_pscal+1) = d
           if(Tp_temp .ge. 1650.d0) uold(ind_cell,firstindex_pscal+2) = d
#else
     write(*,*)'NPSCAL must >= 2 for CAI calculations'
     call clean_stop
#endif
#endif
     !--------------------------------------------------------------------
