...
!#if NPSCAL>=2
     ...
     do i=1,nleaf
       ...
       call temperature_eos(d,e,Ttemp,ht)
     !  T = uold(ind_leaf(i),ndim+2) - ekk(i) - err(i) - emag(i)
     !  T = T2(i)*nH(i)/scale_T2/(gamma-1.0) 
       if(Ttemp .ge. 8.) then
           uold(ind_leaf(i),firstindex_pscal+1) = 1.
           if(myid .EQ. 1) write(*,*) 'transform passive scalar' 
       end if
       uold(ind_leaf(i),firstindex_pscal+1) = 100.*d
       !if(Ttemp .ge. 100.) uold(ind_leaf(i),firstindex_pscal+2) = 1.
       uold(ind_leaf(i),firstindex_pscal+2) = Ttemp*d
     end do
!#else
