...
subroutine compute_clump_properties_round2(xx)
...
#endif
  use hydro_parameters, ONLY:ndust, firstindex_ndust
  ...
  integer::grid,nx_loc,ix,iy,iz,ind,icpu,idim,idust
...
subroutine surface_int_np(ind_cell,np,ilevel)
  ...
  integer::j,ind,nx_loc,i2,j2,k2,ix,iy,iz,idim,jdim,i3,j3,k3,idust
...
!!$                    P_neigh=(gamma-1.0)*(uold(cell_index(j),ndim+2)-ekk_neigh-emag_neigh-err_neigh)
                    ...
                    enddo
#endif                    
                    call pressure_eos((1.0_dp-sum_dust)*d,eint,P_neigh)
...
#if NDUST>0
                       do idust=1,ndust
                          sum_dust= sum_dust+ uold(cell_index(j),firstindex_ndust+idust)/max(uold(cell_index(j),1),smallr)
                       enddo
#endif                                           
                       call pressure_eos((1.0_dp-sum_dust)*max(uold(cell_index(j),1),smallr),eint,P_neigh)
