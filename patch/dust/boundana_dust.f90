...
!############################################################
!############################################################
subroutine boundana_dust(x,u,dx,ibound,ncell)
  ...
  real(dp),dimension(1:nvector,1:ndim,1:ndust)::u ! Conservative variables
  ...
  do idust=1,ndust
     do idim=1,ndim
     do i=1,ncell
        u(i,idust,idim)=boundary_var(ibound,ivar)
     end do
  end do
  end do
