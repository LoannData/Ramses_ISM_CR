...
subroutine virtual_tree_fine(ilevel)
  ...
  if(tracer)particle_data_width=particle_data_width+8
...
subroutine fill_comm(ind_part,ind_com,ind_list,np,ilevel,icpu)
  ...
  if(tracer)then
     do i=1,np
        ...
        reception(icpu,ilevel)%up(ind_com(i),current_property+7)=epsp(ind_part(i))
     end do
     current_property = current_property + 8
  end if
...
subroutine empty_comm(ind_com,np,ilevel,icpu)
  ...
  if(tracer)then
     do i=1,np
        ...
        epsp(ind_part(i))=emission(icpu,ilevel)%up(ind_com(i),current_property+7)
     end do
     current_property = current_property+8
  end if
