...
subroutine trim_clumps
  ...
  skip_loc(3)=dble(kcoarse_min)
  scale=boxlen/dble(nx_loc)
  dx_min=0.5D0**(nlevelmax-nlevelsheld)*scale/aexp
  ...
  do while (b2 > err2)
     do i=1,n-1
        do j=i+1,n
           ...
           dummy=b2
           b2 = b2 - 2.0*A(j,i)**2
           bar = max(0.5*b2/9._dp,0._dp) !deal with weird optimized arithmetics...
           ! calculate coefficient c and s for Givens matrix
           beta = (A(j,j)-A(i,i))/(2.0*A(j,i))
           coeff = 0.5*beta*(1.0+beta**2)**(-0.5)
           s = (max(0.5+coeff,0.0_dp))**0.5
           c = (max(0.5-coeff,0.0_dp))**0.5
           ! update rows i and j
           do k=1,n
              cs =  c*A(i,k)+s*A(j,k)
              sc = -s*A(i,k)+c*A(j,k)
