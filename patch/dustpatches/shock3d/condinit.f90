...
subroutine condinit(x,u,dx,nn)
  ...
  real(dp)::xn,x0,yn,zn,rsquare,sum_dust, t_stop, sound_speed,rho_0, r_c
      ...
      do i=1, nn
         ...
         sum_dust =0.0d0
         rsquare=  (xn-x0)*(xn-x0)/r_c/r_c + (yn-x0)*(yn-x0)/r_c/r_c+ (zn-x0)*(zn-x0)/r_c/r_c
         do idust = 1, Ndust
            q(i,firstindex_ndust+idust) =  dust_region(2,idust)
         ...
         q(i,7)=B_region(2)
         q(i,8)=C_region(2)
         q(i,nvar+1)=A_region(2)
         q(i,nvar+2)=B_region(2)
         q(i,nvar+3)=C_region(2)
         if(1.lt.rsquare) then
         sum_dust =0.0d0
         do idust = 1, Ndust
            q(i,firstindex_ndust+idust) =  dust_region(1,idust)
            sum_dust= sum_dust+ q(i,firstindex_ndust+idust)
         end do
         q(i,1)=d_region(1)
         q(i,2)=u_region(1)
         ...
         q(i,7)=B_region(1)
         q(i,8)=C_region(1)
         q(i,nvar+1)=A_region(1)
         q(i,nvar+2)=B_region(1)
         q(i,nvar+3)=C_region(1)  
         endif
      end do
