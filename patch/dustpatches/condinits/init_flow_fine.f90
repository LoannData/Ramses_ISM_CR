...
subroutine region_condinit(x,q,dx,nn)
  ...
  integer::i,j,ivar,k,idust
  real(dp)::vol,r,xn,yn,zn,en,sum_dust, t_stop, thermal_speed, sound_speed,rho_0
  real(dp)::radiation_source,pi
  call units(scale_l,scale_t,scale_d,scale_v,scale_nH,scale_T2)
  
  rho_0           = 1.0e4_dp/scale_d
  q(1:nn,1)=rho_0
  q(1:nn,2)=0.0d0
  q(1:nn,3)=0.0d0
  q(1:nn,4)=0.0d0
  ...
  pi = 3.14159265358979323_dp
  sound_speed            = SQRT(kb* Temp_dust/ (mu_gas * mH))
  thermal_speed          = SQRT(8*kb*Temp_dust/ (pi*mu_gas*mH))
  t_stop                 = rho_grain * size_grain /thermal_speed/rho_0
  t_stop = t_stop/ scale_t
  sound_speed = sound_speed /scale_l *scale_t
  do idust = 1, Ndust
  CALL Theoretical_values_non_linear (q(1:nn,firstindex_ndust+idust),dx&
       &, 1.0e5_dp,nn,1e-8_dp,t_stop*(sound_speed*sound_speed))
  end do
...
SUBROUTINE Theoretical_values_non_linear (eps_th,dx, t, n_cells, dust_quant,K)
  use amr_parameters
  use hydro_commons
#if NPSCAL>0
  use hydro_parameters,only:var_region
...
REAL (dp),dimension(n_cells), intent(out)   :: eps_th
REAL (dp), intent(in)                       :: dx,t,dust_quant,K
REAL (dp)                                   :: eta_0, alpha, beta, delta, A, lambda_x
INTEGER                                     :: i_x

   beta  = 1.0_dp
   alpha =-1.0_dp/(2.0_dp+beta)
   delta = 1.0_dp/(2.0_dp+beta)
   !beware eta_0 cannot be modified simply by changing alpha, beta, delta, look for barenblatt solution (gamma function required)
   eta_0 = (dust_quant /((half*delta*beta)**(1.0_dp/beta)*4.0_dp/3.0_dp))**(1.0_dp/(1.0_dp+ 2.0_dp/beta))
   A  = (half*eta_0*eta_0*delta*beta)

   !WRITE(*, "('eta_0              =', 1pE16.8)")     eta_0
   !WRITE(*, "('alpha              =', 1pE16.8)")     alpha
   !WRITE(*, "('beta               =', 1pE16.8)")     beta
   !WRITE(*, "('delta              =', 1pE16.8)")     delta
   !WRITE(*, "('A                  =', 1pE16.8)")     A

   eps_th= 0
   DO i_x=1, n_cells
   lambda_x = (REAL(i_x,dp)*dx-REAL(n_cells,dp)/2.0_dp*dx)*((t)**(-delta))/eta_0
      IF (abs(lambda_x).le.1) THEN
      eps_th(i_x) = A/K*(t)**alpha *(1-lambda_x**2)**(1.0_dp/beta)
      END IF
     IF (eps_th(i_x) .lt. 0.0_dp) THEN
     eps_th(i_x) =0.0_dp
     END IF
   END DO

END SUBROUTINE Theoretical_values_non_linear
