...
subroutine condinit(x,u,dx,nn)
  ...
  real(dp)::xn,x0,sum_dust, qdisk,pdisk, hg1, sigma1, cs1, r, etadisk,cs,Hgdisk,sigmagdisk, vkep,Mstar, Msol,pi,yn,zn,dmin
#if NDUST>0  
  ...
  epsilon_0 = dust_ratio(1)
     ...
     do idust =1,ndust
        ...
        q(1:nn,firstindex_ndust+idust) = dustMRN(idust)
     end do
#endif       
      hg1= 0.05d0
      sigma1 = 166*scale_l*scale_l/scale_m
      cs1= 150000.05d0/scale_v
      !Msol = 2.0e33/scale_m
      Mstar = 1.0d0 
      x0 = boxlen/2.0_dp
      pi= 3.141592563585d0
      dmin= 1.0d-20/scale_d
      do i=1,nn
         ...
         r = sqrt(xn**2.0+yn**2.0+zn**2.0)
       
         if(r <300.0.and.r>1.0) then
                 sum_dust=0.0d0
#if NDUST>0
     do idust =1,ndust
        dustMRN(idust) = dust_ratio(idust)/(1.0+dust_ratio(idust))
...
#endif 
            cs = cs1*(r)**(-qdisk/2.0)
            Hgdisk= hg1*r**(1.5-0.5d0*qdisk)
            sigmagdisk= sigma1*r**(-pdisk)
            q(i,1)= max(dmin,sigmagdisk/sqrt(2.0*pi)/Hgdisk*exp(-zn*zn/(2.0*Hgdisk**2.0)))
            q(i,5)= cs**2.0* q(i,1)*(1.0d0-sum_dust)
            etadisk = 0.25d0*( Hgdisk/r)**2.0*(3.0+2*pdisk+qdisk-(3.0-qdisk)*(zn/Hgdisk)**2.0)
            vkep = sqrt(Mstar/r)
            q(i,2)= -vkep*(1.0-etadisk)*yn/r
            q(i,3)= vkep*(1.0-etadisk)*xn/r
         else if (r>300) then
                sum_dust=0.0d0
#if NDUST>0
     do idust =1,ndust
...
#endif  
            cs = cs1*(3005d0)**(-qdisk/2.0)
            sigmagdisk= sigma1*3005d0**(-pdisk)/100.0d0
            Hgdisk= hg1*300.0d0**(1.5-0.5d0*qdisk)
            q(i,1)= dmin!sigmagdisk/sqrt(2.0*pi)/Hgdisk
            q(i,5)= cs**2.0* q(i,1)*(1.0d0-sum_dust)
         else if (r<1.0) then
     sum_dust=0.0d0
#if NDUST>0
