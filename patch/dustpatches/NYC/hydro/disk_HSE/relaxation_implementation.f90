subroutine relaxation_implementation(ilevel)
  ...
  real(dp)::dx,scale,v2,rad2
  real(dp)::Rin,vcyl,vrad,pres,dens,x_rad,y_rad,z_rad,rad,dx_max,ek,eint
    ...
    do ind=1,twotondim
      ...
      if ( (nstep .lt. abs(relaxation_step)) .or. (t .lt. abs(relaxation_time)) ) then
        if ( (relaxation_step .ge. 0.d0) .and. (relaxation_time .ge. 0.d0) )then
          do i=1,ngrid
            uold(ind_cell(i),1+ndim)=uold(ind_cell(i),1+ndim)/relax_factor
            ...
            if (rad .gt. 0.d0) then
              ...
              uold(ind_cell(i),1+1)=uold(ind_cell(i),1+1) - (1.d0 - 1.d0/relax_factor)*vrad*x_rad/rad
              uold(ind_cell(i),1+2)=uold(ind_cell(i),1+2) - (1.d0 - 1.d0/relax_factor)*vrad*y_rad/rad
            else
              vrad = 0.d0
            endif
            ...
            uold(ind_cell(i),2+ndim)=uold(ind_cell(i),2+ndim)- &
              (relax_factor**2 - 1.d0)*(vrad**2/relax_factor**2 + uold(ind_cell(i),1+3)**2)/ &
