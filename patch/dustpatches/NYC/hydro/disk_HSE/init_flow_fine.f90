  ...
  !-------------------------------------------------------
     ...
     do igrid=1,ncache,nvector
        ...
        do ind=1,twotondim
           ...
           call condinit(xx,uu,dx_loc,ngrid,first_lmax)
           ! Scatter variables
           do ivar=1,nvar
              do i=1,ngrid
...
#ifndef WITHOUTMPI
    if ((mod(disk_setup,10) == 1) .and. (ilevel .eq. nlevelmax) .and. first_lmax .and. (.not. disk_readTable) ) then
    ...
    else if ((mod(disk_setup,10) == 1.d0) .and. disk_ReadTable .and. first_lmax) then
