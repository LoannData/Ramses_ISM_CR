...
subroutine userflag_fine(ilevel)
  ...
  real(dp)::dx,dx_loc,scale,dx_min,dx_max
  real(dp)::scale_nH,scale_T2,scale_l,scale_d,scale_t,scale_v
  real(dp),dimension(1:3)::skip_loc,offset
  ...
  offset(1) = dx_max*disk_xc
  offset(2) = dx_max*disk_yc
  offset(3) = dx_max*disk_zc
  ...
  do igrid=1,ncache,nvector
     ...
     do ind=1,twotondim
        ...
        if (cen_refine) then
          ...
          ! Aspect ratio cut, no rescaling necessary.
          call geometry_refine_center(xx,ok,ngrid,ilevel)
        endif
...
subroutine geometry_refine_disk(xx,ok,ncell,ilevel)
  ...
  do i=1,ncell
    extra = 1.1
    rcyl2 = max(xx(i,1)**2 + xx(i,2)**2 + (gravity_params(2)/boxlen)**2 + (0.5*1.d0/2.d0**ilevel)**2,(disk_innerR/boxlen)**2)
...
!############################################################
!############################################################
subroutine geometry_refine_center(xx,ok,ncell,ilevel)
  use amr_commons
  use pm_commons
  use hydro_commons
  use poisson_commons
  ...
  dx = 1.01/2.d0**ilevel
  do i=1,ncell
    rcyl2 = xx(i,1)**2 + xx(i,2)**2
    if ( rcyl2 <= dx**2) then
      ok(i) = .true.
    endif
  enddo

end subroutine geometry_refine_center
!############################################################
!############################################################
