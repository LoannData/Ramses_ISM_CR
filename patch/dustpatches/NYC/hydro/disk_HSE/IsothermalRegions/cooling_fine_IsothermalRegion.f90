subroutine cooling_fine(ilevel)
  ...
  use hydro_commons
  use cooling_module
  use hydro_parameters, only : disk_R0
#ifdef grackle
  ...
  do igrid=1,ncache,nvector
     ...
     do i=1,ngrid
       rcyl(i) = sqrt( (xx(i,1) - (boxlen/2.d0 + disk_dx))**2 + (xx(i,2) - (boxlen/2.d0 + disk_dy))**2 )
     enddo
     
     call coolfine1(ind_grid,ngrid,ilevel,rcyl)
  end do
...
subroutine coolfine1(ind_grid,ngrid,ilevel)
  ...
  real(kind=8),dimension(1:nvector),save::T2min,T2min_rad,Zsolar,boost
  ...
  do ind=1,twotondim
     ...
     if(neq_chem) then
        ...
        if(isothermal .or. isothermal_rad)cooling_on(1:nleaf)=.false.
     endif
     ...
     if (isothermal_rad) then
       do i=1,nleaf
          T2min_rad(i) = T2min_rad(i)*nH(i)/scale_T2/(gamma-1.0)*disk_R0/r_cyl(i)
       end do
     endif
     ...
     else if(isothermal_rad)then
        do i=1,nleaf
           uold(ind_leaf(i),ndim+2) = T2min_rad(i) + ekk(i) + err(i) + emag(i)
        end do
