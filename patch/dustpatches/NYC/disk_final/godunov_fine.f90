...
subroutine set_uold(ilevel)
  ...
  do ind=1,twotondim
     iskip=ncoarse+(ind-1)*ngridmax
     do i=1,active(ilevel)%ngrid
        !first update the passive scalar to 'correct' them if d<smallr
        !this is not conservative but better than negative dust ratios or temperature right?
        !anyways smallr is very small, this happens a very limited number of time during a simulation
        do ivar = firstindex_pscal,lastindex_pscal
          ! if(unew(active(ilevel)%igrid(i)+iskip,1).le.smallr) then
          !    old_pscal=uold(active(ilevel)%igrid(i)+iskip,ivar)/uold(active(ilevel)%igrid(i)+iskip,1)
     ...
     uold(active(ilevel)%igrid(i)+iskip,1) = max(unew(active(ilevel)%igrid(i)+iskip,1),smallr)
     end do             
     do ivar=2,firstindex_pscal
        do i=1,active(ilevel)%ngrid
             uold(active(ilevel)%igrid(i)+iskip,ivar) = unew(active(ilevel)%igrid(i)+iskip,ivar)
        end do
     end do
     do ivar=lastindex_pscal,nvar+3
        do i=1,active(ilevel)%ngrid
             uold(active(ilevel)%igrid(i)+iskip,ivar) = unew(active(ilevel)%igrid(i)+iskip,ivar)
        end do
     end do
...
#if NDUST>0
              do idust=1,ndust
                 sum_dust= sum_dust+ uold(ind_cell,&
                      &firstindex_ndust+idust)/d
              enddo

#endif  
              call enerint_eos((1.0_dp-sum_dust)*d,0.3d0*Tr_floor,e_prim)
...
#if RESIST>0
  call mag_unsplit(uloc,gloc,dc,flux,emfx,emfy,emfz,tmp,dx,dx,dx,dtnew(ilevel),ncache,ind_grid,jcell)
#else
