subroutine relaxation_implementation(ilevel,nstp)
  ...
  real(dp):: v_kep,cs_iso,old_Temp,new_Temp,ht,tauc
  ...
  do igrid=1,ncache,nvector
    ...
    do ind=1,twotondim
      ...
      do i=1,ngrid
         ...
         if(equilibrium_temp.eqv..true.) then
            Teq=cs**2.0*scale_v**2*(mu_gas*mH/(gamma*kb))

            !old internal energy
            enint=uold(ind_cell(i),5)-e
            !old temp
            call  temperature_eos(d*(1.0d0-sum_dust),enint,old_temp,ht)
            new_temp= exp(- dtnew(ilevel)/Tauc)*(old_temp-Teq)+Teq
            cs = sqrt(gamma*kb*new_temp/(mu_gas*mH))/scale_v
         endif
