module radiation_parameters
  ...
  real(dp)::Teff_starsink1!=5000.
  real(dp)::r_starsink1!=1.
  real(dp)::rho_disk0!=2.874d-18
  character(6)::irradiation_test='None' ! To be set between Kuiper or Pinte
  real(dp)::dtmax=1.d100
...
function planck_ana(dens,Tp,Tr,igroup)
  ...
  x = log10(dens)
  y = log10(Tp)
  if(igroup==1 .and. stellar_photon)y=log10(Teff_starsink1) !for stellar_photon approach
...
subroutine init_opacities_pascucci
  ...
  if(irradiation_test == 'Kuiper') then
     ...
     rho_grain  = 3.6_dp
     opafname   = 'kappa_DraineLee_1984.dat'
  ...
  do while(.true.)
     read(46,*,end=556) oplambda(i),dum1,dum2,dum3
     opknul(i) = dum1
     !if (irradiation_test == 'Pinte') opknul(i) = dum2 !dum1 ! dum1 + dum2 !dum2 for abs only with pinte, test
     i = i + 1
  enddo
  556 continue
  close(46)
  ...
  do i = 1,n
     oplambda(i) = oplambda(i)*1.0e-04_dp
     if(irradiation_test == 'Kuiper') then
        opnu(i) = clight / oplambda(i)
        opknu(i) = opknul(i) * grain_size**2 / grain_mass / 100_dp !for pasc/kuip, dust to gas ratio
     else if(irradiation_test == 'Pinte') then
        opnu(n-i+1) = clight / oplambda(i) !because reverse order in kappa WD but not necessary
        opknu(n-i+1) = opknul(i)/ 100.0_dp ! for pinte. /100 because dust to gas ratio?
     endif
