...
!!$ben           norm_er=sqrt(norm_er)!*vol_loc
           ...
           !more restrictive condition: and => or. May not cvg. Raph. Back to non restrictive condition of Benoit
           do while((error/error_ini>epsilon .or. error_cg_loc> epsilon) .and. error_ini .gt. 0.0d0) !to remove after checking if it's responsible for diff in E, L btw _ok and _modif
!           do while(error/error_ini>epsilon .and. error_cg_loc> epsilon)!error_ini/norm_er .gt. 1.0d-15)                                                 
              iter=iter+1
              if(iter>itermax)exit
             !less restrictive than the restrictive condition
             !if(error/error_ini < epsilon .and. error_cg_loc < 1.0e4*epsilon) exit
             !if(error/error_ini < 1.0e4*epsilon .and. error_cg_loc < epsilon) exit
