...
if(ndim>2) then
  ...
  !   firstindex_frad = nvar
  !endif

  !ivar = 1
  !write(ilun,'("variable #",I2,": frad_x",I1)')firstindex_frad+ivar!,ivar should not be here, take example on velocity
  !ivar = 2
  !write(ilun,'("variable #",I2,": frad_y",I1)')firstindex_frad+ivar
  !ivar = 3
  !write(ilun,'("variable #",I2,": frad_z",I1)')firstindex_frad+ivar
  !raph pour force radiative
#endif
...
#else
  write(ilun)nvar+4 !+ndim raph pour frad FLD only? No because RT=1 even with FLD
#endif
  write(ilun)ndim
