...
subroutine rt_make_boundary_hydro(ilevel)
  ...
  ! -------------------------------------------------------------------
  integer::ibound,boundary_dir,idim,inbor,incbor
  integer::i,ncache,ivar,igrid,ngrid,ind
  integer::iskip,iskip_ref,gdim,nx_loc,ix,iy,iz,iskipnb_ref,tmp
  integer,dimension(1:8)::ind_ref,alt
  integer,dimension(1:nvector),save::ind_grid,ind_grid_ref
  integer,dimension(1:nvector),save::ind_cell,ind_cell_ref,ind_cellnb_ref
  ...
  do ibound=1,nboundary
     ...
     ! Joki: Compute index offset for neighbour cell beyond neighbor cell
     if(boundary_dir==1)incbor=1
     if(boundary_dir==2)incbor=-1
     if(boundary_dir==3)incbor=2
     if(boundary_dir==4)incbor=-2
     if(boundary_dir==5)incbor=4
     if(boundary_dir==6)incbor=-4
     ...
     do igrid=1,ncache,nvector
        ...
        do ind=1,twotondim
           ...
           tmp=ind_ref(ind)-1+incbor
           if(tmp.lt.0 .or. tmp .gt.7) print*,'Problem: ',tmp,ind_ref(ind)-1,boundary_dir
           iskipnb_ref=ncoarse+(ind_ref(ind)-1+incbor)*ngridmax !joki
           do i=1,ngrid
              ind_cell_ref(i)=iskip_ref+ind_grid_ref(i)
              ind_cellnb_ref(i)=iskipnb_ref+ind_grid_ref(i) !joki
           end do
           ...
           if((boundary_type(ibound)/10).ne.2)then
              ...
              do ivar=1,nrtvar
                 do i=1,ngrid
                    !uu(i,ivar)=rtuold(ind_cell_ref(i),ivar) !DEBUG joki
                    !uu(i,ivar)=rtuold(ind_cell_ref(i),ivar) *0.9 !DEBUG joki (=31/32)
                    !uu(i,ivar)=rtuold(ind_cell_ref(i),ivar) *0.965 !DEBUG joki (=31/32)
                    !uu(i,ivar)=rtuold(ind_cell_ref(i),ivar) *0.962 !DEBUG joki (=31/32)
                    !uu(i,ivar)=rtuold(ind_cell_ref(i),ivar) *0.9825 !DEBUG joki (=64/65)
                    !uu(i,ivar)=0d0
                    uu(i,ivar)=2d0*rtuold(ind_cell_ref(i),ivar) & ! Joki: lin. extrapolation
                         - rtuold(ind_cellnb_ref(i),ivar)
                    uu(i,ivar)=max(uu(i,ivar),0d0)
                 end do
              end do
