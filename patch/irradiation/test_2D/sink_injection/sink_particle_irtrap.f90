                   ...
                   ! Tstar = Teff_sink(isink)
                    if(Tstar .gt. 0)then
                       ...
                       if(rt_protostar_m1)then
                          ...
                          rtunew(ind_cell(i),5)=rtunew(ind_cell(i),5) + Lum_group*weight*dtnew(ilevel)/((group_egy(2)*ev_to_erg)/scale_d/scale_v**2)/(8.0d0*rmax**3)!((4.0d0*pi*rmax**3)/3.0d0)
                          !rtunew(ind_cell(i),5)=rtunew(ind_cell(i),1) + Lum_group*weight*dtnew(ilevel)/((group_egy(1)*ev_to_erg)/scale_d/scale_v**2)/(8.0d0*rmax**3)
                       end if
#endif
                    end if
